...
class PostsList extends React.Component {
    ...
    componentDidMount() {
        //console.log("__Component Mounted");
        this.attachScrollListener();
        this.initiatePinnedCollapsedState();
    }

    componentDidUpdate(prevProps) {
        //console.log("__Component Updated: Category: ", this.props.category);
        //console.log("__Component Updated: Previous: ", prevProps.category);
        if (this.props.category && this.props.category.startsWith('hive-') && this.props.category !== prevProps.category) {
            this.updateSlide(0);
            //console.log("Need to check collapsed state");
            this.initiatePinnedCollapsedState();
        }
    }
    ...
    updateSlide(index) {
        ...
        let totalSlides = this.props.posts.filter(post => post.getIn(['stats', 'is_pinned'], false)).size;
        ...
        const pinnedPostsElements = document.getElementsByClassName('pinnedPosts');
        const sliderPosition = (index + totalSlides) % totalSlides;
        this.setState({ currentSlide: sliderPosition });
        if (pinnedPostsElements[0]) {
            pinnedPostsElements[0].style.setProperty(
                `--pinned`,
                `-${sliderPosition * 50}%`
            );
        }
    ...
    initiatePinnedCollapsedState() {
        const arePinnedPostsCollapsedStored = process.env.BROWSER ? localStorage.getItem('collapsepinned-'+this.props.category) : null;
        const arePinnedPostsCollapsed = arePinnedPostsCollapsedStored ? JSON.parse(arePinnedPostsCollapsedStored) : false;

        //console.log("Initiating Pinned State as:", arePinnedPostsCollapsed);
    ...
    togglePinnedPosts() {
        ...
        //console.log("Start State:", arePinnedPostsCollapsed);

        const updatedCollapsedState = !arePinnedPostsCollapsed;

        //console.log("New State Should be: ", updatedCollapsedState);

        this.setState(prevState => ({
            arePinnedPostsCollapsed: !prevState.arePinnedPostsCollapsed
        }));

        //console.log("New State Is:", this.state.arePinnedPostsCollapsed);

        if (process.env.BROWSER) {
            localStorage.setItem('collapsepinned-'+category, updatedCollapsedState);
        }
    ...
    render() {
        ...
        const pinnedPosts = posts.filter(post => post.getIn(['stats', 'is_pinned'], false));
        ...
        const renderSummary = items =>
            items.map((post, i) => {
                ...
                const summary = [];
                summary.push(
                    <li key={i} className={post.getIn(['stats', 'is_pinned'], false) ? "isPinned" : ""}>
                        {ps}
                    </li>
                );
        ...
        //console.log("Current State Is:", this.state.arePinnedPostsCollapsed);
        //console.log("In Storage Is: ", localStorage.getItem('collapsepinned-'+this.props.category));

        return (
            <div id="posts_list" className="PostsList">
                {category && category.startsWith('hive-') && pinnedPostsCount > 0 && (order === 'trending' || order ==='created') &&
                    <div className={`${arePinnedPostsCollapsed ? 'pinnedPostsContainer' : ''}`}>
                        ...
                        {arePinnedPostsCollapsed && pinnedPostsCount > 2 &&
                            <div className="carouselDots">{renderDotLinks(pinnedPostsCount)}</div>
                        }
                        {pinnedPostsCount >= 2 &&
                            <div className="collapseShowPinned">
                                <a href="#" 
                                    ...
                                    {pinnedPostsCount == 2 && arePinnedPostsCollapsed ? 'View As List \u25BC' : (
                                        arePinnedPostsCollapsed ? 'Show All Pinned \u25BC' : 'Collapse Pinned \u25B2'
                                    )}
                                </a>
                            </div>
                        }
                ...
                <ul
                    ...
                    itemType="http://schema.org/blogPosts"
                >
                    {category && category.startsWith('hive-') && (order === 'trending' || order ==='created') ? renderSummary(posts.filter(post => !post.getIn(['stats', 'is_pinned'], false))) : renderSummary(posts)}
                </ul>
                {loading && (
                    <center>
                        <LoadingIndicator
